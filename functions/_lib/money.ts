...
/**
 * Единственная учётная валюта админки.
 *
 * Владелец ведёт дела только в долларах — ни рублей, ни сумов. Поэтому выбора
 * валюты в интерфейсе нет, а всё новое сохраняется в USD.
 *
 * Что при этом НЕ делается и почему:
 *
 * - записи, сохранённые раньше в другой валюте, не переписываются и не
 *   пересчитываются: курсов в системе нет, а поменять подпись, не тронув
 *   число, — значит соврать в отчёте;
 * - показ по-прежнему берёт валюту из записи, а суммы по-прежнему считаются
 *   по каждой валюте отдельно. Когда всё в долларах, это просто одна строка,
 *   зато старая запись остаётся честной, а не складывается с долларами.
 */
