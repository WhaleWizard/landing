...
function renderSecurityTxt(): Response {
  ...
  const body = [
    'Contact: mailto:whalewzrd@gmail.com',
...
function buildCsp(): string {
  const directives = [
    ...
    // поэтому GTM и прочие домены здесь не нужны.
    //
    // Facebook — исключение, и оно доказано консолью живого сайта: пиксель
    // отправляет событие обычным GET-пикселем, но когда параметров много и
    // адрес перестаёт помещаться в лимит длины URL, fbevents.js переключается
    // на отправку формой на https://www.facebook.com/tr/. С `form-action 'self'`
    // такие события молча блокировались — по три штуки на одну загрузку
    // главной, — и терялись именно самые богатые из них.
    //
    // На защиту это не влияет: тот же адрес уже разрешён в connect-src, то есть
    // сценарий с внедрённым скриптом отправил бы туда данные и без формы.
    // Запрет не закрывал утечку, а только ломал трекинг.
