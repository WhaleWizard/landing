...
/**
 * Отправляет серверное событие Lead в Meta Conversions API.
 */
async function sendMetaConversionEvent(
  ...
  // Разделяем имя на first name и last name
  const nameParts = (payload.name || '').trim().split(/\s+/);
  const firstName = nameParts[0] || '';
  const lastName = nameParts.slice(1).join(' ') || '';

  // Хешируем все персональные данные (Meta требует SHA256)
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    // Асинхронно отправляем серверное событие в Meta (без ожидания ответа)
