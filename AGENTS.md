...
**Сегодня** (`AdminToday.tsx` + `api/admin/today.ts`): стартовый экран. «Фокус дня» собирает конкретные дела с именами и сроками — просроченные следующие шаги по сделкам, задачи CRM на сегодня и просроченные, новые заявки и невыполненные задачи планера за текущий день (неделя планера лежит одним JSON, нужный день достаётся в коде). Плитки, график за 14 дней и последние заявки читаются из `api/admin/stats`; отдельного компонента дашборда больше нет.
