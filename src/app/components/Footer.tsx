...
function Footer() {
  ...
  const scrollToSection = useCallback((id: string) => {
    ...
    if (location.pathname !== '/') {
      // Переход на главную отдаём её собственному разбору хеша — так же, как
      // это делает шапка. Прежний вариант переходил на «/» и один раз пробовал
      // прокрутить через 120 мс: секции главной монтируются лениво и за это
      // время обычно не успевают, поэтому человек по ссылке из подвала попадал
      // на верх главной вместо нужного блока. Хеш решает это надёжно — Home
