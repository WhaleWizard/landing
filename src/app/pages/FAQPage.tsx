...
export const FAQ_SEO = {
  title: 'FAQ о таргете и Google Ads: бюджет, сроки, аналитика, приложения',
  description: 'Ответы о рекламе в Instagram, Facebook и Google: какой бюджет нужен на тест, сколько стоит ведение, когда ждать заявки, как настроить аналитику и продвигать мобильное приложение.',
...
const faqSeeds: FaqSeed[] = [
  ...
  {
    category: 'Старт',
    question: 'С какими странами и рынками вы работаете?',
    answer: 'Работаю удалённо с русскоязычными клиентами из любой страны, а реклама может идти на любой рынок: СНГ, Европу, США, ОАЭ, Латинскую Америку.',
    details: [
      'Кампании настраиваются там, где живёт ваша аудитория: язык креативов и посадочных подбираем под рынок.',
      'В кейсах на сайте есть проекты на Казахстан, Узбекистан, страны Латинской Америки и Ближнего Востока.',
      'Условия работы и оплаты описаны в публичной оферте на сайте.',
    ],
  },
  {
    category: 'Старт',
    question: 'Как выбрать таргетолога и что у него спросить?',
    answer: 'Спросите, по какому показателю он будет оценивать результат, как передаст в Meta и Google данные о заявках и продажах и что произойдёт, если первый тест не сойдётся по экономике.',
    details: [
      'Честный ответ на третий вопрос важнее красивых кейсов: у рекламы есть период теста, и его результат не гарантирован.',
      'Попросите показать, как устроена аналитика в текущих проектах: события, дедупликация, статусы из CRM.',
      'Если подрядчик уже есть и непонятно, что он делает, — на консультации разбираю кабинет и отчёты и говорю, что спросить.',
    ],
  },
  {
    category: 'Старт',
  ...
  {
    category: 'Бюджет',
    question: 'Сколько стоит настройка и ведение рекламы?',
    answer: 'Стоимость ведения зависит от рекламного бюджета, числа каналов и объёма работы с креативами и аналитикой; ориентир по вашим цифрам даёт калькулятор на сайте, точную сумму называю после первичного разбора.',
    details: [
      'Рекламный бюджет оплачивается отдельно — напрямую в кабинеты Meta и Google.',
      'Единого прайса нет намеренно: поисковая кампания для одной услуги и запуск приложения на три страны — разный объём работы.',
      'Единственная фиксированная цена на сайте — консультация: $70 за встречу 60–90 минут.',
    ],
  },
  {
    category: 'Бюджет',
  ...
  {
    category: 'Процесс',
    question: 'Как проходит первичный разбор рекламы?',
    answer: 'Вы оставляете ссылку на сайт или приложение, контакты и диапазон бюджета; я смотрю кабинет и кампании, если реклама уже идёт, и отвечаю, что исправил бы в первую очередь и какой тест запустил бы первым.',
    details: [
      'Для разбора действующей рекламы достаточно доступа на просмотр к кабинету и аналитике.',
      'Ответ приходит в Telegram или на почту — с приоритетами, а не с общим списком «всё переделать».',
      'Если для честного теста не хватает спроса или бюджета, скажу это до старта.',
    ],
  },
  {
    category: 'Процесс',
  ...
  {
    category: 'Приложения',
    question: 'Сколько стоит установка приложения в Meta Ads?',
    answer: 'Цена установки зависит от страны, категории приложения, креативов и события, на которое оптимизируется кампания; в опубликованных кейсах она отличается в разы между рынками, поэтому единой цифры нет.',
    details: [
      'Дешёвая установка сама по себе не цель: сравниваем её со стоимостью регистрации, покупки или подписки.',
      'На старте считаем допустимую цену платящего пользователя от экономики продукта, а уже от неё — допустимый CPI.',
      'Реальные цифры для вашего приложения и рынка даёт только тест — по ним и решаем, масштабировать ли кампанию.',
    ],
  },
  {
    category: 'Приложения',
    question: 'Можно ли рекламировать приложение, если оно только вышло и событий ещё нет?',
    answer: 'Да: пока данных мало, кампания оптимизируется на установку или первое частое событие, а на покупки и подписки переходим, когда их наберётся достаточно.',
    details: [
      'До запуска проверяем, что установка, регистрация и покупка доходят до Meta через SDK, MMP или Conversions API.',
      'Креативы строим вокруг реального сценария в приложении с первого дня, а не после накопления статистики.',
      'Порог перехода на глубокое событие фиксируем заранее, чтобы не менять цель кампании на глаз.',
    ],
  },
  {
    category: 'Приложения',
...
const FAQ_ID_BY_QUESTION: Record<string, string> = {
  ...
  'С какими странами и рынками вы работаете?': 'geography-and-markets',
  'Как выбрать таргетолога и что у него спросить?': 'how-to-choose-a-specialist',
  'Сколько стоит настройка и ведение рекламы?': 'service-pricing',
  'Как проходит первичный разбор рекламы?': 'initial-review-process',
  'Сколько стоит установка приложения в Meta Ads?': 'app-install-cost',
  'Можно ли рекламировать приложение, если оно только вышло и событий ещё нет?': 'app-ads-without-events',
